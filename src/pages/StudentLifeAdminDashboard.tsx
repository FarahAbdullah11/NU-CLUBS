...
const StudentLifeAdminDashboard: React.FC<DashboardProps> = ({ onLogout }) => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Verify user is STUDENT_LIFE_ADMIN only - redirect SU_ADMIN to their dashboard
  ...
  // Immediate check on mount - redirect SU_ADMIN users
  useEffect(() => {
    const userDataStr = localStorage.getItem('userData');
    if (userDataStr) {
      try {
  ...
  // Early return if user is SU_ADMIN - prevent rendering
  const userDataStr = localStorage.getItem('userData');
  if (userDataStr) {
    try {
      const userData = JSON.parse(userDataStr);
...
export default StudentLifeAdminDashboard;
